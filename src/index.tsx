...
const mergeAll = (...args: any[]) =>
  args.reduce((result, arg) => ({ ...result, ...arg }), {})
...
export const getDataProvider = <
  T extends DataClientBase
>(): GetDataProviderResult<T> => {
  const SSRDataContext = createContext<SSRDataContextBase & T>(
    {} as SSRDataContextBase & T
  )
  ...
  if (isServer()) {
    ...
    const context: SSRDataContextBase & T = ({
      ...
      getScript: async (): Promise<string> => {
        ...
        const dataRequests = context!.ssrDataRequests!.map(
          async ({ request, name }: any) => {
            return {
              [name]: await request
            }
          }
    ...
    const result: GetDataProviderResult<T> = {
      DataProvider: ({ children, providers, dataContext }) => {
        ...
        return (
          <SSRDataContext.Provider value={context}>
            {children}
    ...
    const initialData:
      | { prefetchedState: { [key: string]: any } }
      | undefined = getInitialState()
    const oneTimeCache = initialData ? initialData.prefetchedState : {}
...
export const useDataEffect = async (
